...
void priority_boost() { // solution for priority inversion

	srand((unsigned) time(0));
	while (!ready[GREEN_Q].empty()) { // boost low priority
		TCB *thread = getReady(GREEN_Q);
		if (thread->getLockCount() == 0) {
			addToReady(thread); // add back to original queue
		} else {
			int flag = rand() % 2; // set a flag of 0 or 1
			if (flag) {
	...
	while (!ready[ORANGE_Q].empty()) { // boost medium priority
		TCB *thread = getReady(ORANGE_Q);
		if (thread->getLockCount() == 0) {
			addToReady(thread); // add back to original queue
		} else {
			int flag = rand() % 2; // set a flag of 0 or 1
...
// static int uthread_create_low(void* (*start_routine)(void*), void* arg)
// {
// 	//can't add any more!!
// 	if(_threads.size() == 100)
// 	{
...
// static int uthread_create_high(void* (*start_routine)(void*), void* arg)
// {
// 	//can't add any more!!
// 	if(_threads.size() == 100)
// 	{
